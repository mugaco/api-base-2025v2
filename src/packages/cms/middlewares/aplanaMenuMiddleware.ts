...
import { MenuFlattener, CMSTranslationFlattener, normalizeLocale } from '@packages/cms/utils';
import { Container } from '@core/Container';
import { ILoggerService } from '@core/services/LoggerService';

/**
 * Middleware para aplanar respuestas de Menu en endpoints específicos
 * Solo aplana en rutas / y /:_id usando el sistema CMS de aplanado
 */
export const aplanaMenuMiddleware = (req: Request, res: Response, next: NextFunction): void => {
    ...
    res.json = function (body: unknown): Response {
        ...
        logger.info('[Menu] Locale detectado', { rawLocale, locale });

        // Solo aplanar si tenemos un locale válido
        if (!locale) {
            logger.warn('[Menu] No se detectó locale válido, devolviendo respuesta sin aplanar');
        ...
        if (Array.isArray(transformedData)) {
            transformedData = (transformedData as unknown[]).map((item: unknown) => {
                const typedItem = item as Record<string, unknown> & { toJSON?: () => unknown; menus?: unknown[] };
                // Verificar que el item tenga menus
                if (typedItem.menus && Array.isArray(typedItem.menus)) {
                    ...
                    const plano = MenuFlattener.flatten(plainItem as never, locale);
                    return plano;
                }
                // Si no tiene menus, retornar el item como objeto plano con transform
                return typedItem.toJSON ? typedItem.toJSON() : item;
            });
        } else if (transformedData && (transformedData as Record<string, unknown>).menus) {
            // Si es un solo elemento (no array) pero tiene menus
            ...
            transformedData = MenuFlattener.flatten(plainData as never, locale);
        }
        
        transformedBody.data = transformedData;
        logger.debug('[Menu] Body aplanado', { transformedBody });
