...
import { TagFlattener, CMSTranslationFlattener, normalizeLocale } from '@packages/cms/utils';
import { Container } from '@core/Container';
import { ILoggerService } from '@core/services/LoggerService';

/**
 * Middleware para aplanar respuestas de Tag en endpoints específicos
 * Solo aplana en rutas / y /:_id usando el sistema CMS de aplanado
 */
export const aplanaTagMiddleware = (req: Request, res: Response, next: NextFunction): void => {
    ...
    res.json = function (body: unknown): Response {
        ...
        logger.info('[Tag] Locale detectado', { rawLocale, locale });

        // Solo aplanar si tenemos un locale válido
        if (!locale) {
            logger.warn('[Tag] No se detectó locale válido, devolviendo respuesta sin aplanar');
        ...
        if (Array.isArray(transformedData)) {
            transformedData = (transformedData as unknown[]).map((item: unknown) => {
                const typedItem = item as Record<string, unknown> & { toJSON?: () => unknown; tags?: unknown[] };
                // Verificar que el item tenga tags
                if (typedItem.tags && Array.isArray(typedItem.tags)) {
                    ...
                    const plano = TagFlattener.flatten(plainItem as never, locale);
                    return plano;
                }
                // Si no tiene tags, retornar el item como objeto plano con transform
                return typedItem.toJSON ? typedItem.toJSON() : item;
            });
        } else if (transformedData && (transformedData as Record<string, unknown>).tags) {
            // Si es un solo elemento (no array) pero tiene tags
            ...
            transformedData = TagFlattener.flatten(plainData as never, locale);
        }
        
        transformedBody.data = transformedData;
        logger.debug('[Tag] Body aplanado', { transformedBody });
